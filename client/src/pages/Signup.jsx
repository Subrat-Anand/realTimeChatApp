        ...
        <div className='w-full max-w-[500px] h-[600px] bg-white rounded-lg
         shadow-gray-400 shadow-lg flex flex-col gap-[10px]'>
            ...
            <form className='flex flex-col w-full gap-[20px] items-center'>
                <input type='text' placeholder='username' className='w-[90%] h-[60px]
                ...
                hover:shadow-inner' onClick={handleSignUp} type='submit' disabled={loading}>
                    {loading ? "loading" : "SignUp"}
                </button>

                <p>Already Have An Account ? <span onClick={()=> navigate('/login')} className='text-[#20c7ff] text-[bold] cursor-pointer'>Login</span></p>
            </form>
        </div>
    </div>
  );
